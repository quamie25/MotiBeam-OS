...
Guardian mode with inactivity monitoring
"""
...
class SecurityDemo(MotiBeamScene):
    def __init__(self):
        super().__init__(title="MotiBeam - Security & Guardian")
        self.pulse = 0
    ...
    def render(self):
        ...
        # Header
        self.draw_header("GUARDIAN MODE ACTIVE", "Wellness & Safety Monitoring")
        
        # Alert icon with pulse effect
        pulse_size = 80 + int(20 * abs(pygame.math.Vector2(1, 0).rotate(self.pulse * 57.3).x))
        alert_center = (self.width // 2, 280)
        ...
        alert_text_rect = alert_surf.get_rect(center=(self.width//2, 420))
        ...
        action_rect = action_surf.get_rect(center=(self.width//2, 620))
